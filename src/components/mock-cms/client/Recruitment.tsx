    ...
    <div className="flex flex-col min-h-screen bg-slate-50">
      ...
      style={{ 
        ...
        <div className="max-w-7xl mx-auto px-6 py-20 lg:py-24 flex flex-col items-center text-center">
          ...
          <h1 className="text-4xl lg:text-5xl font-display font-bold mb-6 leading-tight text-white">
          ...
          <div className="w-full max-w-2xl relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-4 rounded-xl border outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 transition-all bg-[var(--surface-secondary)] focus:bg-[var(--surface)]"
              style={{
                borderColor: 'var(--border)',
                color: 'var(--text)',
              }}
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {benefits.map((benefit, index) => (
              <div
                key={index}
                className="p-6 rounded-2xl border hover:shadow-lg transition-shadow duration-300"
                style={{
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
            <div>
              ...
              <p style={{ color: 'var(--text-secondary)' }}>
                Tìm kiếm cơ hội phù hợp với chuyên môn của bạn.
              </p>
            </div>

            <div className="flex gap-2">
              <button
                className="px-4 py-2 rounded-lg text-sm font-medium transition-colors hover:bg-[var(--hover)]"
                style={{
          ...
          <div className="space-y-4">
            {filteredJobs.length > 0 ? (
              filteredJobs.map((job) => (
                <div
                  key={job.id}
                  className="group flex flex-col md:flex-row md:items-center justify-between p-6 rounded-2xl border hover:shadow-md transition-all duration-300 cursor-pointer"
                  style={{
                    background: 'var(--surface)',
                    borderColor: 'var(--border)',
                  }}
                >
                  <div className="mb-4 md:mb-0">
                    <div className="flex items-center gap-3 mb-2">
                      <h3
                        className="text-lg font-bold group-hover:text-[var(--primary)] transition-colors"
                        style={{ color: 'var(--text)' }}
                      >
                        {job.title}
                      </h3>

                      {/* HOT badge - keep original accent */}
                      {job.isHot && (
                  ...
                  <Link
                    ...
                    className="inline-flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl font-medium transition-colors"
                    style={{
