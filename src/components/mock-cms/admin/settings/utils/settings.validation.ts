...
export const validateGeneralInfo = (
  data: GeneralInfo
): boolean => {
  if (!data.logo?.trim()) {
  ...
  if (
    data.websiteName &&
    data.websiteName.length > 60
  ) {
    alert(
      'Tên website không được vượt quá 60 ký tự.'
    );
    return false;
  }
  ...
  if (
    ...
    !isValidPhone(data.companyPhoneNumber)
  ) {
    alert(
  ...
  for (const [name, value] of socialLinks) {
    if (value && !isValidUrl(value)) {
      alert(
        `${name} link không đúng định dạng URL.`
      );
      return false;
    }
  }
  ...
  if (
    data.mapEmbedUrl &&
    !isValidUrl(data.mapEmbedUrl)
  ) {
    alert(
      'Google Maps Embed URL không đúng định dạng.'
    );
    return false;
  }
