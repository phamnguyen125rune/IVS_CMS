...
export function useGeneralInfo() {
  ...
  const fetchGeneralInfo =
    useCallback(async (): Promise<GeneralInfo | null> => {
      try {
        setLoading(true);

        const response =
          await settingService.getGeneralInfo();
  ...
  const openFooterModal = async () => {
    try {
      ...
      console.error(
        'Lỗi khi tải dữ liệu Footer:',
        error
      );

      alert(
        'Không thể tải dữ liệu hiện tại.'
      );
    } finally {
  ...
  const handleSave = async (
    ...
    try {
      ...
      const payload: UpdateGeneralInfoRequest = {
        ...
        websiteDescription:
          data.websiteDescription?.trim() || '',

        email:
          data.email?.trim() || '',

        facebookLink:
          data.facebookLink?.trim() || '',
        ...
        linkedinLink:
          data.linkedinLink?.trim() || '',

        youtubeLink:
          data.youtubeLink?.trim() || '',

        zaloLink:
          data.zaloLink?.trim() || '',

        companyPhoneNumber:
          data.companyPhoneNumber?.trim() || '',
        ...
        footerLinks:
          data.footerLinks?.trim() || '',
      };

      const response =
        await settingService.updateGeneralInfo(
          payload
        );

      if (!response?.data) {
        alert(
          'Cập nhật thông tin website thất bại.'
        );
        return;
      }
      ...
      console.error(
        'Lỗi lưu thông tin chung:',
        error
      );

      alert(
        'Không thể cập nhật thông tin website.'
      );
    } finally {
      setSaving(false);
    }
