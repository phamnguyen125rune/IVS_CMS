...
export function useUsers() {
  ...
  const loadUsers = useCallback(async () => {
    ...
    const remaining = await Promise.all(
      Array.from({ length: pages - 1 }, (_, index) =>
        requestJson<UserPaginationResponse>(
          `/api/users?page=${index + 2}&size=${PAGE_FETCH_SIZE}`
        )
      )
    );
  ...
  async function uploadEditingAvatar(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const data = await requestJson<UserAvatarResponse>(`/api/users/${editingUser.userId}/avatar`, {
        method: 'POST',
        body: formData,
      });
