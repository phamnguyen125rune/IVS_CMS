...
export default function Posts() {
  ...
  // State cho Modals
  const [viewPost, setViewPost] = useState<(typeof initialPosts)[0] | null>(
    null,
  );
  const [deletePost, setDeletePost] = useState<(typeof initialPosts)[0] | null>(
    null,
  );
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center justify-end gap-1">
                        {post.status === "pending" && (
                          <button
                            onClick={() =>
                              navigate(`/admin/kiem-duyet?id=${post.id}`)
                            }
