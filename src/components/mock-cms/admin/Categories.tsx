...
function CategoryItem({ item, depth = 0 }: any) {
  ...
  return (
    <div>
      ...
      >
        <button onClick={() => setExpanded(!expanded)} className="w-5 flex-shrink-0">
          {hasChildren ? (
            expanded ? (
        ...
        <span className="text-xs text-slate-400 font-mono">{item.posts} bài</span>
        ...
        >
          ...
          <div className="p-2">
            {activeTab === 'categories' ? (
              initialCategories.map((cat) => <CategoryItem key={cat.id} item={cat} />)
            ) : (
              <div className="flex flex-wrap gap-2 p-2">
                {tags.map((tag) => (
                  ...
                  >
                    ...
                    <span className="text-xs text-slate-400 font-mono">{tag.posts}</span>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white rounded-xl border p-5" style={{ borderColor: 'var(--border)' }}>
            <h3 className="text-sm font-semibold text-slate-800 mb-4">
              {activeTab === 'categories' ? 'Thêm danh mục mới' : 'Thêm thẻ mới'}
            </h3>
              ...
              {[
                {
                  label: 'Tổng danh mục',
                  value:
                    initialCategories.length + initialCategories.flatMap((c) => c.children).length,
                },
