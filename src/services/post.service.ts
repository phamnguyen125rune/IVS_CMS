...
export class PostService {
    async getPosts(filter: ReqPostFilterDTO, page: number = 1, size: number = 10): Promise<PaginatedResponse<ResPostListDTO>> {
        ...
        const res = await apiFetch<any>(`/api/v1/posts?${params.toString()}`);
        return res.data || res;
    }

    async getPostById(id: number): Promise<ResPostDTO> {
        const res = await apiFetch<any>(`/api/v1/posts/${id}`);
        return res.data || res;
    }

    async createPost(payload: ReqPostCreateDTO): Promise<ResPostDTO> {
        const res = await apiFetch<any>('/api/v1/posts', {
            method: 'POST',
    ...
    async reviewPost(postId: number, payload: ReqPostReviewDTO): Promise<any> {
        const res = await apiFetch<any>(`/api/v1/posts/${postId}/reviews`, {
            method: 'POST',
            body: JSON.stringify(payload)
        });
        return res.data || res;
