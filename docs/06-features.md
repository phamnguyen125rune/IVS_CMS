...
| ---------------------------------------- | ---------------------------------------------------------------- | ------------ |
| **Xác thực (JWT)**                       | `src/app/[language]/(auth)/login/page.tsx`                       | PLACEHOLDER  |
|                                          | `src/app/api/auth/login/route.ts`                                | PLACEHOLDER  |
|                                          | `src/app/api/auth/logout/route.ts`                               | PLACEHOLDER  |
|                                          | `src/proxy.ts` — auth guard                                      | ✅ Có cơ bản |
| **Quản lý phân quyền (RBAC)**            | `src/types/index.ts` — `Role`, `User`                            | PLACEHOLDER  |
|                                          | `src/app/[language]/(admin)/users/page.tsx` — danh sách user     | PLACEHOLDER  |
|                                          | Java: `controller/UserController.java`                           | PLACEHOLDER  |
| **Quản lý Profile**                      | `src/app/[language]/(admin)/profile/page.tsx`                    | PLACEHOLDER  |
| **Quản lý nội dung (CRUD bài viết)**     | `src/app/[language]/(admin)/posts/page.tsx`                      | PENDING      |
|                                          | `src/app/[language]/(admin)/posts/[id]/edit/page.tsx`            | PENDING      |
...
| **Quản lý danh mục (Categories & Tags)** | `src/app/[language]/(admin)/categories/page.tsx`                 | PENDING      |
|                                          | `src/app/[language]/(admin)/tags/page.tsx`                       | PENDING      |
|                                          | `src/types/index.ts` — `Category`, `Tag`                         | PLACEHOLDER  |
| **Thư viện media (Grid view)**           | `src/app/[language]/(admin)/media/page.tsx`                      | PENDING      |
...
| **Quản lý thông tin liên hệ**            | `src/app/[language]/(admin)/contacts/page.tsx`                   | PENDING      |
| **Cấu hình hệ thống (Settings)**         | `src/app/[language]/(admin)/settings/page.tsx`                   | PENDING      |
...
| Tính năng                        | File / Thư mục                                                                     | Trạng thái      |
| -------------------------------- | ---------------------------------------------------------------------------------- | --------------- |
| **Hiển thị nội dung động**       | `src/app/[language]/(client)/news/page.tsx`                                        | PLACEHOLDER     |
|                                  | `src/app/[language]/(client)/news/[slug]/page.tsx` — chi tiết                      | PENDING         |
...
| **Đa ngôn ngữ (i18n)**           | `src/app/[language]/` — locale prefix trong URL                                    | ✅ Cấu trúc sẵn |
|                                  | `src/proxy.ts` — tự redirect theo Accept-Language                                  | ✅ Có           |
|                                  | NOTE: Thư viện i18n (next-intl) chưa cài, tích hợp sau                             | —               |
| **Giới thiệu doanh nghiệp**      | `src/app/[language]/(client)/about/page.tsx`                                       | PLACEHOLDER     |
|                                  | Nội dung: lịch sử, sứ mệnh, tầm nhìn, đội ngũ                                      | —               |
| **Danh mục sản phẩm / dịch vụ**  | `src/app/[language]/(client)/products/page.tsx`                                    | PLACEHOLDER     |
|                                  | `src/app/[language]/(client)/products/[slug]/page.tsx` — chi tiết                  | PENDING         |
| **Tin tức & Sự kiện**            | `src/app/[language]/(client)/news/page.tsx`                                        | PLACEHOLDER     |
|                                  | `src/app/[language]/(client)/news/[slug]/page.tsx` — chi tiết                      | PENDING         |
...
| **Liên hệ & Bản đồ**             | `src/app/[language]/(client)/contact/page.tsx`                                     | PLACEHOLDER     |
